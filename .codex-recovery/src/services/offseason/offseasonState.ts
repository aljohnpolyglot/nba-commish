...
export function getOffseasonState(
  ...
  const openingNight = getOpeningNightDate(upcomingSeasonYear, ls, schedule);
...
export function getVisibleOffseasonRows(
  ...
  if (isEuroIsolatedMode({ leagueStats })) {
    return ['options', 'qualifyingOffers', 'myFAs', 'freeAgency', 'sponsorRenewals', 'facilityUpgrades', 'preseasonFriendlies', 'trainingCamp'];
  }
