        ...
        // Track single-game franchise records from sim (per team, per category)
        {
            ...
            for (const r of simPatch.results) {
                ...
                for (const { stats, teamId, oppId } of rSides) {
                    const team = resolveAnyTeam(teamId, stateWithSim.teams, stateWithSim.nonNBATeams ?? []);
                    const opp = resolveAnyTeam(oppId, stateWithSim.teams, stateWithSim.nonNBATeams ?? []);
                    if (!team) continue;
                    for (const stat of stats) {
