...
export const GameProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const cMonthForInitial = state.date ? new Date(state.date).getUTCMonth() + 1 : 0;
    const isInitialImportWindow = cMonthForInitial >= 7;
    const isInitialFirstSeason = (!state.seasonHistory || state.seasonHistory.length === 0) && isInitialImportWindow;
...
const actions = useGameActions(setState, () => stateRef.current);
  ...
  const dispatchAction = async (action: UserAction) => {
    ...
    if (action.type === 'OFFSEASON_AUTO_RESOLVE_ALL') {
      ...
      setState(prev => {
        if (!prev.offseasonChecklist) return prev;
        const nextChecklist = { ...prev.offseasonChecklist };
        for (const row of Object.keys(nextChecklist) as OffseasonChecklistRow[]) {
          if (nextChecklist[row] === 'pending' || nextChecklist[row] === 'in-progress') {
            nextChecklist[row] = 'skipped';
          }
        }
        return { ...prev, offseasonChecklist: nextChecklist };
      });
    ...
    if (action.type === 'OFFSEASON_EXIT') {
      ...
      const scheduledPreseason = (stateRef.current.schedule ?? [])
        ...
        .sort()[0];
      const scheduledOpeningNight = (stateRef.current.schedule ?? [])
        .filter((g: any) =>
          !g.played &&
          !g.isPreseason &&
          !g.isExhibition &&
        ...
        .filter((d: string) => !!d && (!todayStr || d > todayStr))
        .sort()[0];
      const target = useAssistantGM
        ? (scheduledOpeningNight ?? `${preseasonYear}-10-24`)
        : (scheduledPreseason ?? scheduledOpeningNight ?? `${preseasonYear}-10-01`);
