...
export const useRulesState = (leagueStats: LeagueStats, dispatchAction: (action: any) => Promise<void>) => {
  ...
  const [cupPrizePoolEnabled, setCupPrizePoolEnabled] = useState(leagueStats.cupPrizePoolEnabled ?? true);
  const [cupPrizePoolAutoInflate, setCupPrizePoolAutoInflate] = useState(leagueStats.cupPrizePoolAutoInflate ?? true);
  const [cupPrizeWinner, setCupPrizeWinner] = useState(leagueStats.cupPrizeWinner ?? 500_000);
  const [cupPrizeRunnerUp, setCupPrizeRunnerUp] = useState(leagueStats.cupPrizeRunnerUp ?? 200_000);
  const [cupPrizeSemi, setCupPrizeSemi] = useState(leagueStats.cupPrizeSemi ?? 100_000);
  const [cupPrizeQuarter, setCupPrizeQuarter] = useState(leagueStats.cupPrizeQuarter ?? 50_000);
...
const [minAgeRequirement, setMinAgeRequirement] = useState<number | string>(leagueStats.minAgeRequirement ?? 19);
  ...
  const [customScheduleEnabled, setCustomScheduleEnabled] = useState(leagueStats.customScheduleEnabled ?? false);
  ...
  // Default OFF — real 2026 NBA tournament uses 12-min total games, not the
  // standard 4×12 that mirroring league rules would produce.
  const [allStarMirrorLeagueRules, setAllStarMirrorLeagueRules] = useState(leagueStats.allStarMirrorLeagueRules ?? false);
  ...
  const [allStarShootingStars, setAllStarShootingStars] = useState(leagueStats.allStarShootingStars ?? false);
  ...
  // The Throne — premium 1v1 tournament
  const [allStarThroneEnabled, setAllStarThroneEnabled] = useState(leagueStats.allStarThroneEnabled ?? false);
  const [allStarThroneFieldSize, setAllStarThroneFieldSize] = useState<8 | 16>((leagueStats.allStarThroneFieldSize ?? 16) as 8 | 16);
  const [allStarThroneFormat, setAllStarThroneFormat] = useState<'mini8' | 'full16'>(leagueStats.allStarThroneFormat ?? 'full16');
  const [allStarThroneFirstPossession, setAllStarThroneFirstPossession] = useState<'shootout' | 'higher_seed_choice'>(leagueStats.allStarThroneFirstPossession ?? 'shootout');
  const [allStarThroneScoring, setAllStarThroneScoring] = useState<'2s_and_3s' | '1s_and_2s'>(leagueStats.allStarThroneScoring ?? '2s_and_3s');
  const [allStarThroneShotClock, setAllStarThroneShotClock] = useState(leagueStats.allStarThroneShotClock ?? 7);
  const [allStarThroneTargetScore, setAllStarThroneTargetScore] = useState(leagueStats.allStarThroneTargetScore ?? 12);
  const [allStarThroneHardCap, setAllStarThroneHardCap] = useState(leagueStats.allStarThroneHardCap ?? 16);
  const [allStarThronePrizePool, setAllStarThronePrizePool] = useState(leagueStats.allStarThronePrizePool ?? 5_000_000);
  const [allStarThroneMandatoryDefense, setAllStarThroneMandatoryDefense] = useState(leagueStats.allStarThroneMandatoryDefense ?? true);

  // Rising Stars & Celebrity Game
  const [risingStarsEnabled, setRisingStarsEnabled] = useState(leagueStats.risingStarsEnabled ?? true);
  // Default: 4-team bracket tournament (matches 2026 NBA tournament format and All-Star Game default).
  const [risingStarsFormat, setRisingStarsFormat] = useState(leagueStats.risingStarsFormat ?? '4team_tournament');
  const [risingStarsMirrorLeagueRules, setRisingStarsMirrorLeagueRules] = useState(leagueStats.risingStarsMirrorLeagueRules ?? false);
  const [risingStarsQuarterLength, setRisingStarsQuarterLength] = useState(leagueStats.risingStarsQuarterLength ?? 3);
  const [risingStarsEliminationEndings, setRisingStarsEliminationEndings] = useState(leagueStats.risingStarsEliminationEndings ?? true);
  ...
  // Economy - Finances
  const [currency, setCurrency] = useState(leagueStats.currency ?? 'USD');
  const [tradesAllowed, setTradesAllowed] = useState(leagueStats.tradesAllowed ?? true);
  const [euroMinSalaryUSD, setEuroMinSalaryUSD] = useState(leagueStats.euroMinSalaryUSD ?? EURO_ISOLATED_DEFAULTS.euroMinSalaryUSD);
  const [euroMaxSalaryUSD, setEuroMaxSalaryUSD] = useState(leagueStats.euroMaxSalaryUSD ?? EURO_ISOLATED_DEFAULTS.euroMaxSalaryUSD);
  const [euroleagueBRL, setEuroleagueBRL] = useState(leagueStats.euroleagueBRL ?? EURO_ISOLATED_DEFAULTS.euroleagueBRL);
  ...
  const [tradeMatchingRatioUnder, setTradeMatchingRatioUnder] = useState(leagueStats.tradeMatchingRatioUnder ?? 1.25);
  const [tradeMatchingRatioOver1st, setTradeMatchingRatioOver1st] = useState(leagueStats.tradeMatchingRatioOver1st ?? 1.10);
  const [tradeMatchingRatioOver2nd, setTradeMatchingRatioOver2nd] = useState(leagueStats.tradeMatchingRatioOver2nd ?? 1.00);
  const [restrictCashSendOver2ndApron, setRestrictCashSendOver2ndApron] = useState(leagueStats.restrictCashSendOver2ndApron ?? true);
  const [restrictAggregationOver2ndApron, setRestrictAggregationOver2ndApron] = useState(leagueStats.restrictAggregationOver2ndApron ?? true);
  ...
  const [stepienRuleEnabled, setStepienRuleEnabled] = useState(leagueStats.stepienRuleEnabled ?? true);
  ...
  useEffect(() => {
    const isDifferent = 
        ...
        allStarThroneFieldSize !== (leagueStats.allStarThroneFieldSize ?? 16) ||
        ...
        euroMinSalaryUSD !== (leagueStats.euroMinSalaryUSD ?? EURO_ISOLATED_DEFAULTS.euroMinSalaryUSD) ||
        euroMaxSalaryUSD !== (leagueStats.euroMaxSalaryUSD ?? EURO_ISOLATED_DEFAULTS.euroMaxSalaryUSD) ||
        euroleagueBRL !== (leagueStats.euroleagueBRL ?? EURO_ISOLATED_DEFAULTS.euroleagueBRL) ||
  ...
  const handleSaveConfig = async () => {
    ...
    try {
        ...
        const cleanedFormat = playoffFormat.map(val => {
            ...
            if (num % 2 === 0) num += 1;
            // After odd-bump, re-clamp in case 7 → 8 → would overshoot (impossible
            // mathematically since 7 stays 7, but explicit for future tweaks).
            if (num > NBA_MAX_PLAYOFF_SERIES) num = NBA_MAX_PLAYOFF_SERIES;
            return num;
        });
        ...
        const cleanedGamesPerSeason = Number.isFinite(gamesPerSeason)
            ? Math.min(NBA_MAX_GAMES_PER_SEASON, Math.max(1, Math.round(gamesPerSeason)))
            : 82;
        let cleanedDivisionGames = Number.isFinite(divisionGames)
            ? Math.min(NBA_MAX_DIVISION_GAMES, Math.max(0, Math.round(divisionGames)))
            : 16;
        let cleanedConferenceGames = Number.isFinite(conferenceGames)
            ? Math.min(NBA_MAX_CONFERENCE_GAMES, Math.max(0, Math.round(conferenceGames)))
            : 36;
        // Ratio enforcement: division + conference cannot exceed total games.
        // Trim conference first (rebuilders care more about division rivalry exposure)
        // so a user who sets too many of both gets a graceful clamp instead of a
        // negative inter-conference count.
...
if (cleanedMinAge !== (leagueStats.minAgeRequirement ?? 19)) changes.push(`Minimum age requirement set to ${cleanedMinAge}`);
        ...
        if (customScheduleEnabled !== (leagueStats.customScheduleEnabled ?? false)) changes.push(`Custom schedule ${customScheduleEnabled ? 'enabled' : 'disabled'}`);
        if (cleanedGamesPerSeason !== (leagueStats.gamesPerSeason ?? 82)) changes.push(`Games per season set to ${cleanedGamesPerSeason}`);
        if (cleanedDivisionGames !== (leagueStats.divisionGames ?? 16)) changes.push(`Division games set to ${cleanedDivisionGames}`);
        ...
        if (allStarThroneEnabled !== (leagueStats.allStarThroneEnabled ?? false)) changes.push(`The Throne (1v1 Tournament) ${allStarThroneEnabled ? 'enabled' : 'disabled'}`);
        if (allStarThroneFieldSize !== (leagueStats.allStarThroneFieldSize ?? 16)) changes.push(`Throne Field Size set to ${allStarThroneFieldSize}`);
        if (allStarThroneFirstPossession !== (leagueStats.allStarThroneFirstPossession ?? 'shootout')) changes.push(`Throne First Possession changed to ${allStarThroneFirstPossession}`);
        if (allStarThroneScoring !== (leagueStats.allStarThroneScoring ?? '2s_and_3s')) changes.push(`Throne Scoring changed to ${allStarThroneScoring}`);
        if (allStarThroneShotClock !== (leagueStats.allStarThroneShotClock ?? 7)) changes.push(`Throne Shot Clock set to ${allStarThroneShotClock}s`);
        if (allStarThroneTargetScore !== (leagueStats.allStarThroneTargetScore ?? 12)) changes.push(`Throne Target Score set to ${allStarThroneTargetScore}`);
        if (allStarThroneHardCap !== (leagueStats.allStarThroneHardCap ?? 16)) changes.push(`Throne Hard Cap set to ${allStarThroneHardCap}`);
        if (allStarThronePrizePool !== (leagueStats.allStarThronePrizePool ?? 5_000_000)) changes.push(`Throne Prize Pool set to $${allStarThronePrizePool.toLocaleString()}`);
        ...
        // Economy Changes
        if (currency !== (leagueStats.currency ?? 'USD')) changes.push(`Currency changed to ${currency}`);
        if (tradesAllowed !== (leagueStats.tradesAllowed ?? true)) changes.push(`Trades ${tradesAllowed ? 'enabled' : 'disabled'}`);
        ...
        if (tradeMatchingRatioUnder !== (leagueStats.tradeMatchingRatioUnder ?? 1.25)) changes.push(`Under-apron salary matching set to ${tradeMatchingRatioUnder.toFixed(2)}x`);
        if (tradeMatchingRatioOver1st !== (leagueStats.tradeMatchingRatioOver1st ?? 1.10)) changes.push(`1st-apron salary matching set to ${tradeMatchingRatioOver1st.toFixed(2)}x`);
        if (tradeMatchingRatioOver2nd !== (leagueStats.tradeMatchingRatioOver2nd ?? 1.00)) changes.push(`2nd-apron salary matching set to ${tradeMatchingRatioOver2nd.toFixed(2)}x`);
        if (restrictCashSendOver2ndApron !== (leagueStats.restrictCashSendOver2ndApron ?? true)) changes.push(`2nd-apron cash-send gate ${restrictCashSendOver2ndApron ? 'enabled' : 'disabled'}`);
        if (restrictAggregationOver2ndApron !== (leagueStats.restrictAggregationOver2ndApron ?? true)) changes.push(`2nd-apron aggregation gate ${restrictAggregationOver2ndApron ? 'enabled' : 'disabled'}`);
        ...
        const newStats = {
            ...
            allStarThroneEnabled,
            allStarThroneFieldSize,
            allStarThroneFormat,
            allStarThroneFirstPossession,
            allStarThroneScoring,
            allStarThroneShotClock,
            allStarThroneTargetScore,
            allStarThroneHardCap,
            allStarThronePrizePool,
            allStarThroneMandatoryDefense,
            risingStarsEnabled,
            risingStarsFormat,
            risingStarsMirrorLeagueRules,
            risingStarsQuarterLength,
            ...
            tradeMatchingRatioUnder,
            tradeMatchingRatioOver1st,
            tradeMatchingRatioOver2nd,
            restrictCashSendOver2ndApron,
            restrictAggregationOver2ndApron,
  ...
  const handleApplyEuroDefaults = () => {
    setCurrency(EURO_ISOLATED_DEFAULTS.currency ?? 'EUR');
    setTradesAllowed(EURO_ISOLATED_DEFAULTS.tradesAllowed ?? false);
    setDraftType(EURO_ISOLATED_DEFAULTS.draftType ?? 'no_draft');
    setEligibilityRule('none');
    setMinAgeRequirement(18);
    setMinGamesRequirement(20);
    setSalaryCap(EURO_ISOLATED_DEFAULTS.salaryCap ?? 45_000_000);
    setSalaryCapEnabled(EURO_ISOLATED_DEFAULTS.salaryCapEnabled ?? false);
    setSalaryCapType(EURO_ISOLATED_DEFAULTS.salaryCapType ?? 'none');
    setMinimumPayrollEnabled(false);
    setMinimumPayrollPercentage(0);
    setLuxuryTaxEnabled(false);
    setLuxuryTaxThresholdPercentage(0);
    setApronsEnabled(false);
    setNumberOfAprons(0);
    setFirstApronPercentage(0);
    setSecondApronPercentage(0);
    setTradeMatchingRatioUnder(1);
    setTradeMatchingRatioOver1st(1);
    setTradeMatchingRatioOver2nd(1);
    setRestrictCashSendOver2ndApron(false);
    setRestrictAggregationOver2ndApron(false);
    setRestrictSignAndTradeAcquisitionOver1stApron(false);
    setFreezePickAt2ndApron(false);
    setRestrictTPEProvenanceOver2ndApron(false);
    setPostSigningMoratoriumEnabled(false);
    setTwoWayContractsEnabled(false);
    setMinPlayersPerTeam(11);
    setMaxPlayersPerTeam(16);
    setMaxStandardPlayersPerTeam(12);
    setMaxTwoWayPlayersPerTeam(0);
    setMaxTrainingCampRoster(20);
    setMinContractType('static');
    setMinContractStaticAmount(0.06);
    setMaxContractType('static');
    setMaxContractStaticPercentage(40);
    setSupermaxEnabled(false);
    setSupermaxPercentage(0);
    setBirdRightsEnabled(false);
    setMaxContractLengthStandard(5);
    setMaxContractLengthBird(0);
    setTenDayContractsEnabled(false);
    setMleEnabled(false);
    setRoomMleAmount(0);
    setNonTaxpayerMleAmount(0);
    setTaxpayerMleAmount(0);
    setBiannualEnabled(false);
    setBiannualAmount(0);
    setRoomMlePercentage(0);
    setNonTaxpayerMlePercentage(0);
    setTaxpayerMlePercentage(0);
    setBiannualPercentage(0);
    setTradableDraftPickSeasons(0);
    setStepienRuleEnabled(false);
    setTradeExceptionsEnabled(false);
    setDisabledPlayerExceptionEnabled(false);
    setRfaMatchingEnabled(false);
    setRfaMatchWindowDays(0);
    setRfaAutoDeclineOver2ndApron(false);
    setFaStartMonth(6);
    setFaStartDay(30);
    setFaMoratoriumDays(0);
    setRookieScaleType('none');
    setRookieStaticAmount(0);
    setRookieMaxContractPercentage(0);
    setRookieContractLength(0);
    setRookieTeamOptionsEnabled(false);
    setRookieTeamOptionYears(0);
    setRookieRestrictedFreeAgentEligibility(false);
    setRookieContractCapException(false);
    setR2ContractsNonGuaranteed(false);
    setQuarterLength(10);
    setFoulOutLimit(5);
    setThreePointLineDistance(22.15);
    setDefensiveThreeSecondEnabled(false);
    setDefensiveThreeSecondValue(0);
    setMaxTimeouts(5);
    setCoachChallenges(false);
    setMaxCoachChallenges(0);
    setAllStarGameEnabled(false);
    setAllStarDunkContest(false);
    setAllStarThreePointContest(false);
    setAllStarShootingStars(false);
    setAllStarSkillsChallenge(false);
    setAllStarHorse(false);
    setAllStarOneOnOneEnabled(false);
    setAllStarThroneEnabled(false);
    setRisingStarsEnabled(false);
    setCelebrityGameEnabled(false);
  };

  const handleResetConfig = () => {
    setPlayIn(leagueStats.playIn);
    setInSeasonTournament(leagueStats.inSeasonTournament ?? true);
    setCupPrizePoolEnabled(leagueStats.cupPrizePoolEnabled ?? true);
    setCupPrizePoolAutoInflate(leagueStats.cupPrizePoolAutoInflate ?? true);
    setCupPrizeWinner(leagueStats.cupPrizeWinner ?? 500_000);
    setCupPrizeRunnerUp(leagueStats.cupPrizeRunnerUp ?? 200_000);
    setCupPrizeSemi(leagueStats.cupPrizeSemi ?? 100_000);
    setCupPrizeQuarter(leagueStats.cupPrizeQuarter ?? 50_000);
    ...
    setCustomScheduleEnabled(leagueStats.customScheduleEnabled ?? false);
    ...
    setAllStarMirrorLeagueRules(leagueStats.allStarMirrorLeagueRules ?? false);
    ...
    setAllStarShootingStars(leagueStats.allStarShootingStars ?? false);
    ...
    setAllStarThroneEnabled(leagueStats.allStarThroneEnabled ?? false);
    setAllStarThroneFieldSize((leagueStats.allStarThroneFieldSize ?? 16) as 8 | 16);
    setAllStarThroneFormat(leagueStats.allStarThroneFormat ?? 'full16');
    setAllStarThroneFirstPossession(leagueStats.allStarThroneFirstPossession ?? 'shootout');
    setAllStarThroneScoring(leagueStats.allStarThroneScoring ?? '2s_and_3s');
    setAllStarThroneShotClock(leagueStats.allStarThroneShotClock ?? 7);
    setAllStarThroneTargetScore(leagueStats.allStarThroneTargetScore ?? 12);
    setAllStarThroneHardCap(leagueStats.allStarThroneHardCap ?? 16);
    setAllStarThronePrizePool(leagueStats.allStarThronePrizePool ?? 5_000_000);
    setAllStarThroneMandatoryDefense(leagueStats.allStarThroneMandatoryDefense ?? true);
    setRisingStarsEnabled(leagueStats.risingStarsEnabled ?? true);
    setRisingStarsFormat(leagueStats.risingStarsFormat ?? '4team_tournament');
    setRisingStarsMirrorLeagueRules(leagueStats.risingStarsMirrorLeagueRules ?? false);
    setRisingStarsQuarterLength(leagueStats.risingStarsQuarterLength ?? 3);
    setRisingStarsEliminationEndings(leagueStats.risingStarsEliminationEndings ?? true);
    ...
    setCurrency(leagueStats.currency ?? 'USD');
    setTradesAllowed(leagueStats.tradesAllowed ?? true);
    setEuroMinSalaryUSD(leagueStats.euroMinSalaryUSD ?? EURO_ISOLATED_DEFAULTS.euroMinSalaryUSD);
    setEuroMaxSalaryUSD(leagueStats.euroMaxSalaryUSD ?? EURO_ISOLATED_DEFAULTS.euroMaxSalaryUSD);
    setEuroleagueBRL(leagueStats.euroleagueBRL ?? EURO_ISOLATED_DEFAULTS.euroleagueBRL);
    ...
    setTradeMatchingRatioUnder(leagueStats.tradeMatchingRatioUnder ?? 1.25);
    setTradeMatchingRatioOver1st(leagueStats.tradeMatchingRatioOver1st ?? 1.10);
    setTradeMatchingRatioOver2nd(leagueStats.tradeMatchingRatioOver2nd ?? 1.00);
    setRestrictCashSendOver2ndApron(leagueStats.restrictCashSendOver2ndApron ?? true);
    setRestrictAggregationOver2ndApron(leagueStats.restrictAggregationOver2ndApron ?? true);
    ...
    setStepienRuleEnabled(leagueStats.stepienRuleEnabled ?? true);
  ...
  // ── Aggregate accessor: (rules, setRule) pattern ──────────────────────────
  // New consumer pattern that lets components avoid declaring 30-100 prop/setProp
  // pairs. `rules` is the current effective LeagueStats (state values overlaid on
  // saved leagueStats); `setRule(key, val)` dispatches to the right setter by
  // naming convention — `setX` for state `x`. Build flat object first, then
  // synthesize from it so the dispatch stays in lockstep with the return shape.
  const flatState = {
    playIn, setPlayIn,
    inSeasonTournament, setInSeasonTournament,
    cupPrizePoolEnabled, setCupPrizePoolEnabled,
    cupPrizePoolAutoInflate, setCupPrizePoolAutoInflate,
    cupPrizeWinner, setCupPrizeWinner,
    cupPrizeRunnerUp, setCupPrizeRunnerUp,
    cupPrizeSemi, setCupPrizeSemi,
    cupPrizeQuarter, setCupPrizeQuarter,
    playoffFormat, setPlayoffFormat,
    draftType, setDraftType,
    minAgeRequirement, setMinAgeRequirement,
    eligibilityRule, setEligibilityRule, draftEligibilityRule: eligibilityRule, setDraftEligibilityRule: setEligibilityRule,
    minGamesRequirement, setMinGamesRequirement,
    customScheduleEnabled, setCustomScheduleEnabled,
    ...
    allStarThroneEnabled, setAllStarThroneEnabled,
    allStarThroneFieldSize, setAllStarThroneFieldSize,
    allStarThroneFormat, setAllStarThroneFormat,
    allStarThroneFirstPossession, setAllStarThroneFirstPossession,
    allStarThroneScoring, setAllStarThroneScoring,
    allStarThroneShotClock, setAllStarThroneShotClock,
    allStarThroneTargetScore, setAllStarThroneTargetScore,
    allStarThroneHardCap, setAllStarThroneHardCap,
    allStarThronePrizePool, setAllStarThronePrizePool,
    allStarThroneMandatoryDefense, setAllStarThroneMandatoryDefense,
    ...
    risingStarsQuarterLength, setRisingStarsQuarterLength,
    risingStarsEliminationEndings, setRisingStarsEliminationEndings,
    ...
    tradesAllowed, setTradesAllowed,
    euroMinSalaryUSD, setEuroMinSalaryUSD,
    euroMaxSalaryUSD, setEuroMaxSalaryUSD,
    euroleagueBRL, setEuroleagueBRL,
    ...
    stepienRuleEnabled, setStepienRuleEnabled,
  ...
  // Synthesize { rules, setRule } from flatState by walking the entries.
  // Convention: each pair of entries `(x, setX)` in flatState becomes one rules
  // field. setRule looks up the setter by key name. Drift between key + setter
  // is impossible since they're declared together in the literal above.
  const rules = {} as LeagueStats;
  const setterByKey = {} as Record<string, (v: any) => void>;
  for (const [key, value] of Object.entries(flatState)) {
    if (key.startsWith('set') && key.length > 3 && typeof value === 'function') {
      const fieldKey = key.charAt(3).toLowerCase() + key.slice(4);
      setterByKey[fieldKey] = value as (v: any) => void;
    } else {
      (rules as any)[key] = value;
    }
  }
  const setRule = <K extends keyof LeagueStats>(key: K, val: LeagueStats[K]): void => {
