...
export function JumpView({ event, players, gameSeed, onFinish, isPaused }: JumpViewProps) {
  ...
  const [lmsFailsCount, setLmsFailsCount] = useState<Record<string, number>>({});
  const [lmsHistory, setLmsHistory] = useState<Record<string, string>>({}); // status like XO-
  const [lmsSurvivors, setLmsSurvivors] = useState<string[]>([]);
  const [lmsHeights, setLmsHeights] = useState<number[]>([1.80]);
  const [lmsAttempts, setLmsAttempts] = useState<Record<string, Record<number, string>>>({});
