...
## Session 60 (May 11, 2026) — Euro isolated Spain MVP gates and roster fixes

- `src/components/central/view/TeamOffice/pages/TeamNeeds.tsx` — Team Needs now uses active-league teams plus `isOnRoster`, so Endesa/Euroleague rostered players are included instead of being filtered out by the NBA-only `status === 'Active'` check.
- `src/components/central/view/TeamOffice/TeamOfficeView.tsx`, `src/components/central/view/TeamOffice/pages/TeamIntel.tsx`, `src/components/central/view/LeagueFinancesView.tsx`, `src/components/layout/MainContent.tsx` — trade surfaces are hidden or centrally blocked when `leagueStats.tradesAllowed === false`, including stale Trade Hub/Trade Board/Trade Machine deep-links.
- `src/components/sidebar/NavigationMenu.tsx`, `src/components/layout/MainContent.tsx`, `src/components/central/view/LeagueHistoryView.tsx`, `src/hooks/useDraftEventGate.tsx`, `src/components/offseason/OffseasonAufgaben.tsx` — Euro-isolated mode no longer exposes NBA-only seasonal, draft, playoff, central, Hall of Fame, or legacy-history surfaces; stale routes now land on European competition/schedule views or product-language placeholders.
- `src/components/competition/CompetitionView.tsx`, `src/components/central/view/TeamOffice/pages/TeamIntelFreeAgency.tsx` — competition tables now seed all clubs before games are played, highlight the user club, show qualification status, prize pool, and recent results; the confusing early-season "if season ended today" projection was removed. Euro free-agency scouting hides NBA-only Bird Rights and formats asking prices in the active currency.
- `src/components/schedule/view/components/CalendarView.tsx`, `src/components/schedule/view/components/DayView.tsx` — Euro schedules now render only competition fixtures and suppress NBA Draft/Lottery/All-Star/Playoff event cards in calendar and day views.
- `src/services/competition/competitionScheduler.ts`, `src/store/logic/turn/simulationHandler.ts`, `src/components/central/view/StandingsView.tsx` — Endesa/Euroleague scheduling now uses true round-robin rounds for all AI clubs, centralizes competition team selection including merged Euroleague clubs via `clubAliasMap`, caps seeded tables to the spec team count, and repairs malformed in-flight schedules without deleting already-played results.
- `src/components/shared/PlayButton.tsx` — Euro-isolated saves now get a competition-aware PlayButton branch sourced from `competitionId`/`competitionPhase` fixtures: next fixture, competition regular-season ends, EuroLeague play-in/playoffs/Final Four, and through Endesa/EuroLeague/all competitions. NBA-only skips for trade deadline, All-Star, draft lottery, draft, and NBA playoffs are suppressed in no-draft/Euro saves.
- `src/components/schedule/view/components/CalendarView.tsx`, `src/components/shared/GameSimulatorScreen.tsx`, `src/store/GameContext.tsx`, `src/store/logic/actionProcessor.ts`, `src/components/NewsFeed.tsx`, `src/components/layout/Twitterlayout.tsx` — Euro schedule calendar cells, watched-game screens, watched-game boxscores, news/social photo lookups, and game recap story seeds now resolve Endesa/Euroleague clubs through `resolveAnyTeam`; watched Euro boxscores keep `competitionId`/`competitionPhase` so recaps, standings, and competition history do not lose the game.
- `src/services/competition/competitionResolver.ts`, `src/store/logic/turn/simulationHandler.ts`, `src/store/logic/gameLogic.ts`, `src/services/logic/seasonRollover.ts`, `src/types.ts` — Euro competitions now inject postseason Play-In/QF/SF/Final games into the simulation schedule, resolve real played postseason outcomes when present, keep regular-season tables isolated from KO games, archive typed `competitionHistory`, and write Champion/Runner Up entries into both `history` and `historicalAwards`.
...
**Offseason orchestrator instrumentation landed (Sessions 1 + 2 of multi-step refactor)** — added `src/services/offseason/offseasonState.ts` exporting `getOffseasonState(date, leagueStats, schedule, signals?)` and a throttled `logOffseasonDrift(caller, allowedPhases, actual)` warning helper. Phase enum: `inSeason | preDraft | draftDay | postDraft | moratorium | birdRights | openFA | preCamp`. Derivation reuses the existing `dateUtils` helpers (`getDraftDate`, `getCurrentOffseasonEffectiveFAStart`, `getCurrentOffseasonFAMoratoriumEnd`, `getTrainingCampDate`) so behavior is identical to current logic — this commit only EXPOSES the phase. Drift warnings wired into `faMarketTicker.tickFAMarkets`, `AIFreeAgentHandler.runAIFreeAgencyRound`, `runAIBirdRightsResigns`, and `seasonRollover.applySeasonRollover`. They log when a system fires in a phase where the orchestrator design says it shouldn't (e.g., AI MLE pass during moratorium, Bird Rights re-sign on day 30 of openFA).

**Session 2 — shadow plan landed.** Added `src/services/offseason/offseasonPlan.ts` exporting `getOffseasonDayPlan(state) → OffseasonDayPlan` and a throttled `shadowCompare(subsystem, planned, actuallyFired, plan)` helper. The plan is a pure-function derivation of "what each subsystem should do today" per the orchestrator design — actions for `rollover`, `tickFAMarkets`, `runAIFAPass`, `runBirdRightsPass`, plus the existing `faFrequency` cadence and computed flags (`isFreeAgencySeason`, `moratoriumActive`, `underMinRoster`, `birdRightsAlreadyRanThisYear`, `rolloverAlreadyHappened`). Plan encodes current behavior 1:1 — `rollover` mirrors `shouldFireRollover`'s date check (works with playoff slide), `runAIFAPass` includes `state.day % faFrequency` cadence and `underMinRoster` bypass.

**Session 3 — plan now authoritative.** The four offseason gates in `simulationHandler.ts` now read directly from `offseasonPlan.actions.X === 'fire'` instead of recomputing. Removed: the rollover gate's `shouldFireRollover()` call, the bird-rights gate's `(isFreeAgencySeason && simMonth === 7 && !moratoriumActiveForDay && passYear !== ls.year)` 4-clause check, the FA market tick's `if (isFreeAgencySeason)` check, and the AI FA round's `(isFreeAgencySeason && !moratoriumActiveForDay && (state.day % faFrequency === 0 || anyTeamUnderMinRoster))` check. All four now route through one orchestrator decision per day.

**Session 4 — dead inline cleanup.** Deleted now-unused locals in `simulationHandler.ts`: `isSummerFAWindow`, `effectiveFAStart`, `summerFAOpen`, `isFreeAgencySeason`, `moratoriumActiveForDay`, `faFrequency`, `minRosterSetting`, `anyTeamUnderMinRoster`. Dropped imports of `shouldFireRollover`, `shadowCompare`, `compareGameDates`, `getCurrentOffseasonEffectiveFAStart`, `isInMoratorium`. Net reduction: ~30 lines of date arithmetic in the hot tick loop. `isRegularSeason` retained — still used by the G-League cleanup loop. The per-function `logOffseasonDrift` warnings inside `tickFAMarkets` / `runAIFAPass` / `runAIBirdRightsResigns` / `applySeasonRollover` from Session 1 remain — they catch cases where some non-orchestrator caller (lazySimRunner, autoResolvers) invokes these functions outside their allowed phase.

**Session 5 — full-coverage tracing + parallel-dispatch unification.** Single searchable tag `[OSPLAN]` added across the entire offseason machinery — grep one identifier in DevTools to see the full chronological dispatch timeline.
- **`lazySimRunner` rollover routed through orchestrator:** the multi-day-skip path was calling `shouldFireRollover` + `applySeasonRollover` directly, parallel to `simulationHandler`'s dispatch. Now both paths read `getOffseasonDayPlan(state).actions.rollover === 'fire'` so they can't disagree on a boundary day. `lazySimRunner` logs `[OSPLAN] lazySimRunner.rollover fire date=...`.
- **Dead `birdResigns: []` block deleted** in `simulationHandler`. The 70-line block at line 1095 always ran through an empty array (intentional disable to prevent pre-moratorium signings). Replaced with a 4-line historical-note comment so future readers understand why bird rights does NOT fire on rollover day.
- **Plan trace self-emitting:** `getOffseasonDayPlan` now emits `[OSPLAN] phase=... date=... fire=... reason=...` whenever the dispatch decisions change (de-duped to suppress same-day redundant calls).
- **Drift warnings unified:** `logOffseasonDrift` ([Session 1) and `shadowCompare` (Session 2) now both prefix with `[OSPLAN]` so they appear in the same grep stream as the trace.
- **External signing router instrumented:** `routeUnsignedPlayers` (Oct 1 overseas routing) gets a drift check + `[OSPLAN]` log so we catch it firing outside the `preCamp` phase. The Oct 1 event block in `simulationHandler` (LOYAL retirement + routing) gets `[OSPLAN] simulationHandler.preCampOct1 fire date=...`.
- **`logPlanEvent(caller, action, extra?)` exported** from `offseasonPlan.ts` for any future callsite that wants to mark itself in the timeline without computing a full plan.

**How to use [OSPLAN]:** open DevTools console, filter by `OSPLAN`. Every offseason dispatch decision (plan computation, dispatch fire, drift warning, shadow disagreement) shows up in one stream with phase + date + reason. If something fires at the wrong time, the line right before it tells you why the plan thought it should — or didn't.

**Verification:** `npx tsc --noEmit` passes for all touched files; pre-existing errors in `ThroneContestView.tsx` and `GameContext.tsx` remain.

## Session 51 (May 3, 2026) - Draft/play-button phase leaks, MLE persistence, trade-validator cap absorption, mobile trade modal

**React #185 infinite update loop in DraftSimulatorView fixed** - `computedDraftOrder` was memoized against the full `[state]` object, so every dispatch rebuilt its array reference. A redundant persist `useEffect` then watched that derived `draftOrder` and re-dispatched `activeDraftOrder` on each rebuild, which fed back into `state.draftPicks` updates and re-triggered the memo. Narrowed the memo deps to `[state.leagueStats?.year, state.draftPicks, state.draftLotteryResult, state.teams]` and removed the redundant effect (commitPickToState / simToPickInstant already persist `activeDraftPicks` and `activeDraftOrder` atomically per pick).

**MLE counter no longer ignored on user signings** - `calculateNewStats` (statUpdater.ts:90) was rebuilding `newLeagueStats` from the pre-action `state.leagueStats`, which silently threw away the `mleUsage` increment that `handleSignFreeAgent` writes into `result.leagueStats`. Switched the spread to `result.leagueStats ?? state.leagueStats` so action-handler league-stat patches survive into the final reducer payload. User can no longer spam MLE deals without the available figure decrementing.

**Draft-night PlayButton no longer shows "Through playoffs"** - `getSimPhase()` checked `hasActivePlayoffs || draftBlockedByPlayoffs` before considering `state.draftComplete`, so any leftover series whose `status` never flipped to `'complete'` kept the user stuck in playoff-phase options on draft night. Added an early-return for `state.draftComplete → 'after-draft'` so the offseason chain ("Until FA opens") wins over stale playoff state.

**Watch Lottery surfaces during playoffs/play-in on lottery day** - lottery date typically falls during conf-finals in real NBA. The play-button only exposed "Watch lottery" inside `'draft-lottery'` phase, which is unreachable while playoffs are active. Now `'playoffs'` and `'playin'` phases add a navigate-to-lottery option when `norm === draftLotteryStr`.

**"Until play-in" survives the bracket-not-yet-generated dead window** - if all RS games are played but the play-in bracket hasn't been scheduled, both `findFirstPlayInDate` and `lastRegSeasonStr` are null, so the option vanished and stranded users with only "One day". Added a `${calYear}-04-14` calendar fallback (matches the PlayoffView `${year}-04-20` PLAYIN_END convention) so the option keeps appearing through the gap.

**GM-mode trade-deadline override leak closed** - `TradeSummaryModal` only branched to "Fix Salary to Proceed" for past-deadline + invalid-salary in GM mode. Past-deadline + valid-salary still showed "Override Deadline & Confirm" — a commissioner action leaking into GM mode. Added a top-priority branch that renders a disabled "Trade Deadline Has Passed" pill for `isGM && isPastDeadline`.

**Trade validator now allows cap-spaced facilitator trades** - `cbaTradeRules.ts:checkIncoming` only honored the cap-room exception for the team WITH cap room. The repro: Cleveland (over cap, contender) takes Mitchell ($36.6M) for Stewart ($15M) from Charlotte ($34M cap room, rebuilder) — fails the 1.25× ratio for Cleveland by $17.8M, even though Charlotte's cap space could absorb the imbalance through a 3-team or S&T construction. Added an `otherSideCapRoomUSD` parameter and a final-fallback house rule: when the per-side checks fail but the opposing team's cap room ≥ the salary imbalance, allow the trade. Real NBA would route this through alternative deal structures; the game collapses that into a permitted body-for-pick deal so users can build it directly.

**TeamDropdown auto-sorts by current-season standings** - the shared dropdown used by `TradeMachineModal`, `TradeFinderView`, and `NewsSubNav` previously rendered teams in raw `state.teams` order. Now sorts each conference by win% (with raw wins as tiebreaker), so the dropdown mirrors `StandingsView` ordering everywhere it is used.

**Mobile TradeMachineModal layout/scroll/hover repaired** - hover card horizontal placement used `rect.right + 8` / `rect.left - cardW - 8`, which produced negative `left` values on viewports < 640px and pushed the card off-screen. Now centers horizontally on mobile and clamps vertical placement when the viewport is shorter than the card. The wrapper's `h-[calc(100vh-9rem)]` constraint fought the columns' `min-h-[50vh]` (2×50vh > 91vh) and starved internal scrolling; dropped the fixed mobile height and bumped each column to `min-h-[85vh]` so each panel takes a full screen-worth and the outer `overflow-y-auto` scrolls naturally between Team 1, Team 2, and the action bar.
