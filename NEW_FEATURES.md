...
- **Training & Recovery budget tier (sport-science staff)** — owner-level annual budget GMs allocate across three buckets: **Performance Science** (lower fatigue accumulation rate per training day, faster rest-day recovery — modern NBA teams have cryo, hyperbaric, individualized load monitoring), **Recovery Staff** (lower injury risk multiplier under fatigue — masseuses, sports therapists, sleep specialists), **Player Development** (the Coaching/Training Dev staff entry above). Tiered spending unlocks team-level multipliers that stack on top of the baseline `tickPlayerFatigue` / `injuryRate` / progression formulas. Cheap-skate tier: 90s grind energy (current baseline). Mid tier: typical modern NBA. Elite tier: Heat Culture / Warriors-tier sport-science department where stars stay fresh on Double intensity all year. Integration point: extend `team` with `staffBudget: { performance, recovery, development }` and a per-team multiplier lookup that `trainingTick.tickPlayerFatigue`, `InjurySystem.calculateInjuries`, and `ProgressionEngine.progressPlayer` consult. Surface as a dedicated **Front Office → Sport Science** tab with budget sliders and projected effects ("Your performance tier reduces fatigue accumulation by 25%, recovery by +30%, injury risk under fatigue by 40%"). Why worth building: current fatigue/injury baselines are intentionally conservative (modern athletes have god-tier recovery infrastructure); adding budget tiers lets GMs choose to invest in keeping their stars healthy through grinding seasons, mirroring the real-NBA arms race in performance science staff.
