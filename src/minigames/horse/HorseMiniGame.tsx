...
import { HorseLiveContest } from './HorseLiveContest';
import { horseScore, ratingOf } from './horseScoring';

const MAX_PICKS = 10;
const MIN_PICKS = 3;

export default function HorseMiniGame() {
  const [watching, setWatching] = useState(false);
  const metrics = useCallback((player: any) => [
    { label: '3PT', value: ratingOf(player, 'tp') },
    { label: 'FT', value: ratingOf(player, 'ft') },
    { label: 'HANDLE', value: ratingOf(player, 'drb') },
  ], []);
  const picker = useContestPlayerPicker({
    maxPicks: MAX_PICKS,
    scorePlayer: horseScore,
    metrics,
  });
  ...
  if (watching) {
    return (
      <HorseLiveContest
        contestants={picker.selectedPlayers}
      ...
      <AnimatePresence mode="wait">
        {picker.view === 'LOADING' && (
          <motion.div key="load" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex min-h-screen flex-col items-center justify-center gap-4">
            ...
            <p className="font-mono text-xs uppercase tracking-widest text-zinc-500">Loading shot-makers...</p>
          </motion.div>
        )}
        ...
        {picker.view === 'PICK' && (
          <motion.div key="pick" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mx-auto min-h-screen max-w-7xl p-6">
            <div className="mb-8 flex flex-col gap-5 border-b border-zinc-800 pb-6 md:flex-row md:items-end md:justify-between">
              <div>
                <span className="mb-2 block font-mono text-xs uppercase tracking-widest text-orange-400">All-Star Weekend</span>
                <h1 className="text-4xl font-black uppercase tracking-tighter sm:text-6xl">Pick H-O-R-S-E Field</h1>
                <p className="mt-1 text-sm text-zinc-500">{picker.selectedIds.size}/{MAX_PICKS} selected · minimum {MIN_PICKS}</p>
              </div>
              <button
                onClick={() => setWatching(true)}
                disabled={picker.selectedIds.size < MIN_PICKS}
            ...
            <ContestPlayerSelector items={picker.items} selectedIds={picker.selectedIds} onToggle={picker.toggle} maxSelections={MAX_PICKS} />
            ...
            <div className="mt-8 flex items-center justify-center gap-4 text-center font-mono text-[10px] uppercase tracking-widest text-zinc-700">
              <span><Target className="mr-1 inline h-3 w-3" /> Sandbox mode - no save required</span>
              {selectedNames.length > 0 && (
