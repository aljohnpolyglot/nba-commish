...
export default function TheThroneView() {
  ...
  // Load players from the real NBA roster data
  useEffect(() => {
  ...
  // Auto-shuffler: runs when entering MATCHUP_INTRO with no player2 yet
  useEffect(() => {
    const m = matches[currentMatchIndex];
    if (view !== AppView.MATCHUP_INTRO || !m || m.player2 || unpickedOpponents.length === 0) return;
    setShufflerDone(false);
    setShufflerDisplay(null);
    ...
    const interval = setInterval(() => {
      elapsed += 80;
      // Cycle through random unpicked opponents for visual effect
      setShufflerDisplay(unpickedOpponents[Math.floor(Math.random() * unpickedOpponents.length)]);
      if (elapsed >= totalMs) {
        clearInterval(interval);
        // Lock in the final pick
        const posId = m.player1.id; // higher seed always gets ball
        ...
        {view === AppView.MATCHUP_INTRO && matches[currentMatchIndex] && (() => {
          ...
          return (
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="p-4 md:p-12 max-w-5xl mx-auto min-h-screen flex flex-col justify-center"
            >
              <div className="text-center mb-8">
                <span className="text-yellow-500 font-mono text-xs tracking-widest uppercase">Round {currentRound} · Match {currentMatchIndex + 1}</span>
              </div>

              <div className="grid grid-cols-3 items-center gap-6 md:gap-12 mb-12">
                {/* Player 1 — higher seed */}
                <div className="flex flex-col items-center">
                  <div className="relative w-full max-w-[240px] aspect-[4/5] bg-zinc-900 border-2 border-yellow-500/50 rounded overflow-hidden mx-auto">
                    <img src={m.player1?.imgURL} className="w-full h-full object-cover object-top" alt=""
                      onError={(e) => (e.currentTarget.src = 'https://www.nba.com/assets/img/default-headshot.png')} />
                    <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black to-transparent">
                      <div className="text-yellow-500 font-mono text-[10px] mb-0.5">SEED #{m.player1?.seed}</div>
                      <div className="text-xl font-black italic uppercase leading-none">{m.player1?.lastName}</div>
                    </div>
                    {shufflerDone && (
                      <div className="absolute top-2 right-2 bg-yellow-500 text-black text-[8px] font-black px-1.5 py-0.5 rounded leading-none">BALL</div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center text-center gap-4">
                  <div className="text-6xl md:text-8xl font-black italic text-white/5 tracking-tighter select-none">VS</div>
                  {shufflerDone ? (
                    <div className="flex flex-col gap-3 w-full max-w-[180px]">
                      <button
                        onClick={() => playMatch(m)}
                        className="w-full py-4 bg-yellow-500 text-black font-black uppercase tracking-widest hover:bg-yellow-400 text-sm"
                      >
                        Play Match
                      </button>
                      <button
                        onClick={() => simMatch(handleGameFinish)}
                        className="w-full py-3 border border-zinc-800 text-zinc-500 hover:text-white font-black uppercase text-xs"
                      >
                        Quick Sim
                      </button>
                    </div>
                  ) : (
                    <div className="text-[10px] text-zinc-600 font-mono uppercase tracking-widest animate-pulse">
                      Drawing challenger...
                    </div>
                  )}
                </div>

                {/* Player 2 — shuffler reveal */}
                <div className="flex flex-col items-center">
                  <div className={`relative w-full max-w-[240px] aspect-[4/5] bg-zinc-900 rounded overflow-hidden mx-auto border-2 ${shufflerDone ? 'border-zinc-500/50' : 'border-zinc-800'} transition-all`}>
                    {displayP2 ? (
                      <>
                        <img
                          key={displayP2.id}
                        ...
                        />
                        <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black to-transparent">
                          <div className="text-zinc-500 font-mono text-[10px] mb-0.5">SEED #{displayP2.seed}</div>
                          <div className={`text-xl font-black italic uppercase leading-none ${!shufflerDone ? 'blur-[2px]' : ''}`}>
                            {displayP2.lastName}
                          </div>
                        </div>
                      </>
                    ...
                    {/* Shuffler scan overlay while running */}
