...
export const ChristmasGamesModal: React.FC<ChristmasGamesModalProps> = ({ teams, onClose, onConfirm, initialGames = [] }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 md:p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-slate-900 border border-slate-800 rounded-[2rem] w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[calc(100vh-1.5rem)] md:max-h-[calc(100vh-2rem)]">
        <div className="p-4 sm:p-6 md:p-8 border-b border-slate-800 flex items-start justify-between gap-3 bg-slate-900/50">
          <div className="flex min-w-0 items-start gap-3 sm:gap-4">
            ...
            <div className="min-w-0">
              <h2 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tight">Christmas Day Showcase</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 space-y-6 custom-scrollbar">
          ...
          <div className="space-y-4">
            {games.map((game, index) => (
              <div key={index} className="flex flex-col gap-4 bg-slate-800/50 p-4 rounded-2xl border border-slate-700/50 group animate-in slide-in-from-bottom-2 duration-300" style={{ animationDelay: `${index * 50}ms` }}>
                <div className="flex items-center justify-between gap-3">
                  <h4 className="text-white font-bold text-sm">Matchup {index + 1}</h4>
                  <button 
        ...
        <div className="p-4 sm:p-6 md:p-8 border-t border-slate-800 bg-slate-900/50 flex flex-col-reverse sm:flex-row gap-3 sm:gap-4">
          <button
            onClick={onClose}
