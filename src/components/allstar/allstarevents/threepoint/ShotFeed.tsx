...
export function ShotFeed({ plays, currentIndex, contestants }: ShotFeedProps) {
  const scrollRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const style = document.createElement('style');
    style.textContent = `
      .commentary-feed { scrollbar-width: thin; scrollbar-color: rgba(255,255,255,0.12) transparent; }
      .commentary-feed::-webkit-scrollbar { width: 3px; }
      .commentary-feed::-webkit-scrollbar-track { background: transparent; }
      .commentary-feed::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.12); border-radius: 10px; }
      .commentary-feed::-webkit-scrollbar-thumb:hover { background: rgba(255,255,255,0.25); }
    `;
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <span style={{
                    ...
                    color: style.color, lineHeight: 1.5,
