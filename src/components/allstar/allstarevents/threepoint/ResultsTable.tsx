...
import { ThreePointContestResult } from '../../../../services/allStar/ThreePointContestSim';
...
export function ResultsTable3PT({ contestants, result }: ResultsTable3PTProps) {
  ...
  const rows = contestants.map(c => {
    ...
    return { contestant: c, r1, finals, isWinner, isFinalist, sortScore };
  });
                              ...
                              <div className="flex items-center justify-end space-x-4">
                                ...
                                <div className="flex space-x-2">
                                  {row.r1.stations.map((s, i) => (
                                    <div key={i} className="px-2 py-1 bg-slate-800 rounded text-xs font-mono text-slate-300 border border-slate-700">{s.score}</div>
                                  ))}
                                </div>
                              </div>
                            )}
                              ...
                              <div className="flex items-center justify-end space-x-4">
                                ...
                                <div className="flex space-x-2">
                                  {row.finals.stations.map((s, i) => (
                                    <div key={i} className="px-2 py-1 bg-slate-800 rounded text-xs font-mono text-amber-500 border border-slate-700">{s.score}</div>
                                  ))}
                                </div>
                              </div>
                            )}
