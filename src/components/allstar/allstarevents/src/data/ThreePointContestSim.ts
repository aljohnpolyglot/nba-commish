...
import { ShotChartResult, ShotZoneData } from './shotChartService';
...
export function simulateContest(
  ...
  // Top 3 advance (or top 2? The prompt says "Two shooters left. One champion." in FINALS_HEADER, but usually it's 3. Let's use 2 based on the prompt "announce top 2")
  ...
  // SANDBOX: result stored in local hook state  
  // TRANSPLANT: dispatch({ type: 'ALL_STAR_SIMULATE_3PT', result }) → state.allStar.threePointContest
