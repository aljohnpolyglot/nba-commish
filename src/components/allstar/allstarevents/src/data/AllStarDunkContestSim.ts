...
/**
 * Mocking existing game types and data for the sandbox.
 * In the real app, these would be imported from the core game engine.
 */
...
export const DRIVING_DUNK: Record<string, number> = {
  ...
  "Greg Brown III": 85,
  "Cole Anthony": 82,
  "Juan Toscano-Anderson": 80,
  "Jericho Sims": 84,
  "KJ Martin": 87,
  "Trey Murphy III": 85,
  "Luka Doncic": 25, // Explicitly low as per prompt example
};

/**
 * Mocking external utility functions
 */
export function getBadgeProb(playerName: string, badgeName: string, baseProb: number): number {
  // In a real app, this fetches from the badge system. 
  // For sandbox, we'll return a random level multiplier.
  const levels = [0, 1.0, 1.2, 1.5]; // None, Bronze/Silver, Gold, HOF
...
export class AllStarDunkContestSim {
  ...
  static calcComposite(player: NBAPlayer): number {
    ...
    const drivingDunk = DRIVING_DUNK[player.name];
  ...
  static selectContestants(players: NBAPlayer[], num = 4): NBAPlayer[] {
    const withComposite = players
      .filter(p => DRIVING_DUNK[p.name] !== undefined && DRIVING_DUNK[p.name] >= 40) // Basic gate
    ...
    const picks: NBAPlayer[] = [];
    // Always grab the top elite dunker
    if (elite.length) picks.push(pick(elite.slice(0, 3)).player);
    
    // Add variety
    while (picks.length < num - 1) {
      if (acrobat.length && Math.random() > 0.3) {
        picks.push(pick(acrobat).player);
      } else if (athletic.length) {
        picks.push(pick(athletic).player);
      } else if (elite.length) {
        picks.push(pick(elite).player);
      }
      // Dedupe
      const unique = [...new Set(picks)];
      if (unique.length === picks.length) break;
      picks.length = unique.length;
    }

    // Wildcard — favor past winners even if lower composite
    const pastWinners = withComposite.filter(p => 
      p.player.awards?.some(a => a.type === 'Slam Dunk Contest Winner') &&
      !picks.includes(p.player)
    );
    
    if (pastWinners.length && picks.length < num) {
      picks.push(pick(pastWinners).player);
    } else if (picks.length < num) {
      const remaining = withComposite.filter(p => !picks.includes(p.player));
      if (remaining.length) picks.push(pick(remaining.slice(0, 5)).player);
    }

    const finalSelection = [...new Set(picks)].slice(0, num);
    console.log(`[DunkSim] Final Selection:`, finalSelection.map(p => p.name));
    return finalSelection;
  }

  static simulate(contestants: NBAPlayer[]): DunkContestResult {
    const log: string[] = [];
