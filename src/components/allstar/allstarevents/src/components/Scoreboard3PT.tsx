...
export function Scoreboard3PT({ contestants, plays, currentIndex, ballState, phase }: Scoreboard3PTProps) {
  ...
  // Determine finalists if we are in finals
  ...
  // Track previous score to compute delta
  ...
  return (
    <div className="bg-slate-900/50 rounded-xl border border-slate-800 overflow-hidden flex flex-col">
      <style>{`
        ...
        .scoreboard-feed::-webkit-scrollbar-thumb { 
          background: rgba(255,255,255,0.12); 
          border-radius: 10px; 
        }
        .scoreboard-feed::-webkit-scrollbar-thumb:hover { 
      ...
      <div className="scoreboard-feed flex-1 overflow-y-auto p-2 space-y-2">
        {contestants.map(c => {
          ...
          let opacityClass = isEliminated ? 'opacity-40' : 'opacity-100';
