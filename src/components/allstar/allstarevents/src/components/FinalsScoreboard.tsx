...
import { BallResult } from '../data/ThreePointContestSim';
import { getStationScores } from './Scoreboard3PT';

interface FinalsScoreboardProps {
  finalist1: ThreePointContestant;   // lower R1 score = goes first
  finalist2: ThreePointContestant;   // higher R1 score = goes second
  r1Scores: Record<string, number>;  // round 1 totals for both
  finalsScores: Record<string, number>; // finals running totals
    ...
    <div style={{
      ...
      {isActive && (
        <div style={{
          ...
          <span style={{ fontSize: 9, fontWeight: 600, letterSpacing: '0.06em', color: '#fff' }}>
            LIVE
          </span>
        </div>
      )}
      ...
      <div style={{
        ...
        <img src={contestant.imgURL} style={{
      ...
      }}>{contestant.team} · {contestant.pos}</div>
      ...
      <div style={{
        ...
        borderRadius: 20, width: 'fit-content', margin: '0 auto 10px',
      }}>
      ...
      <div style={{
        display: 'flex', justifyContent: 'center', gap: 5, marginBottom: 4,
      }}>
        {[0,1,2,3,4].map(si => {
      ...
      <div style={{
        ...
        {[0,1,2,3,4].map(si => {
          ...
          return (
            <span key={si} style={{
              color: pts === undefined ? 'transparent'
                : pts >= 5 ? (isMBRack ? '#f59e0b' : '#4ade80')
                : pts >= 3 ? 'rgba(255,255,255,0.5)'
                : '#f87171',
      ...
      <div style={{
        ...
        {scoreDiff > 0 && phase !== 'done' && (
          <span style={{
            ...
            {leadingPlayer.name.split(' ')[1]} +{scoreDiff}
          </span>
        )}
      </div>
