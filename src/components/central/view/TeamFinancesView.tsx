...
export const TeamFinancesView: React.FC = () => {
  ...
  const { firstApron, secondApron } = {
    firstApron: capThresholds.firstApron,
    secondApron: capThresholds.secondApron,
  };
  ...
  const payroll = useMemo(() => getTeamPayrollUSD(state.players, selectedTeamId), [state.players, selectedTeamId]);
  ...
  const playerPieData = useMemo(() => {
    return [...teamPlayers]
      ...
      .map(p => ({
        name: p.name,
        value: contractToUSD(p.contract?.amount || 0),
        pos: p.pos
      }));
  }, [teamPlayers]);

  const highEarners = useMemo(() => {
    return playerPieData.filter(p => p.value >= 8_000_000);
  }, [playerPieData]);
  ...
  const sankeyData = useMemo(() => {
    ...
    const guards = teamPlayers.filter(p => p.pos.includes('G'));
    const forwards = teamPlayers.filter(p => p.pos.includes('F'));
    const centers = teamPlayers.filter(p => p.pos.includes('C'));
            ...
            >
              {state.teams.map(team => (
                <option key={team.id} value={team.id}>
                  {team.city || team.region} {team.name}
                </option>
              ))}
            </select>
            ...
            <div className="bg-[#232730] rounded-xl p-6 border border-slate-800/50">
              ...
              <div className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-500">Below by {formatSalaryM(firstApronSpace)}</span>
                </div>
                <div className="flex items-center gap-2">
                  <AlertTriangle className="w-4 h-4 text-emerald-400" />
                  <span className="font-bold text-emerald-400">Below 2nd Apron</span>
                  <span className="text-slate-500">Below by {formatSalaryM(secondApronSpace)}</span>
                </div>
              </div>
            </div>
                      ...
                      <tr key={player.internalId}>
                        ...
                        <td className="py-1 px-1">
                          <div className="bg-[#facc15] text-slate-900 font-bold text-center py-1.5 rounded">
                            {formatSalaryM(contractToUSD(amount))}
                          </div>
                        </td>
