...
export const TrendItem = ({ category, title, posts, onClick }: { category: string, title: string, posts: string | number, onClick?: () => void }) => (
  <div className="px-4 py-3 hover:bg-white/5 cursor-pointer transition-colors" onClick={onClick}>
...
export const WhoToFollow: React.FC<WhoToFollowProps> = ({ name, handle, avatar, isFollowing, onToggleFollow, onProfileClick, compact }) => (
  <div className="px-4 py-3 hover:bg-white/5 cursor-pointer transition-colors flex items-center justify-between" onClick={onProfileClick}>
    <div className="flex items-center space-x-3 min-w-0">
      <div className="w-10 h-10 rounded-full bg-zinc-800 overflow-hidden flex-shrink-0">
        {avatar ? <img src={avatar} alt={name} className="w-full h-full object-cover" referrerPolicy="no-referrer" /> : <div className="w-full h-full flex items-center justify-center text-white font-bold">{name?.[0] || '?'}</div>}
      </div>
    ...
    <button 
      ...
      className={isFollowing 
        ? "bg-transparent border border-zinc-500 text-white font-bold text-sm px-4 py-1.5 rounded-full hover:bg-rose-500/10 hover:border-rose-500 hover:text-rose-500 transition-colors group flex-shrink-0 ml-2"
