...
export async function loadBadges() {
  ...
  try {
    ...
    if (!res.ok) {
      console.error('[BadgeService] HTTP error! status:', res.status);
      badgesData = {};
      return;
    }
    badgesData = await res.json();
    console.log('[BadgeService] ✅ Badges loaded successfully!');
  } catch (e) {
    clearTimeout(timeoutId);
    console.error('[BadgeService] ❌ Failed to load badges:', e);
    badgesData = {};
  }
}
...
export function getBadgeProb(playerName: string, badgeName: string, baseProb: number) {
