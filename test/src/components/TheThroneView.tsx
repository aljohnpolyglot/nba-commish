...
import { TheThroneGame } from './game/TheThroneGame';
...
export default function TheThroneView() {
  ...
  const startTournament = () => setView(AppView.PLAYER_PICKER);
  ...
  const commitField = () => {
    ...
    setUnpickedOpponents(ranked.slice(8, 16)); // Seeds 9-16 unpicked
          ...
          >
            ...
            <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-lg text-left max-w-sm mx-auto mb-12">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center">
                  <label className="text-xs font-mono text-zinc-400 uppercase tracking-widest">Possession</label>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setSettings({...settings, makeItTakeIt: false})}
                      className={`px-3 py-1 text-[10px] font-black tracking-wider transition-colors ${!settings.makeItTakeIt ? 'bg-yellow-500 text-black' : 'bg-black text-zinc-500 border border-zinc-800'}`}
                    >Loser's</button>
                    <button
                      onClick={() => setSettings({...settings, makeItTakeIt: true})}
                      className={`px-3 py-1 text-[10px] font-black tracking-wider transition-colors ${settings.makeItTakeIt ? 'bg-yellow-500 text-black' : 'bg-black text-zinc-500 border border-zinc-800'}`}
                    >Winner's</button>
                  </div>
                </div>
